    ...
    jq.widget('ui.gestalt', jq.ui.mouse, {
        ...
        destroy: function() {
            ...
            if (this.options.keyboard) {
                jq(document).unbind(jq.browser.mozilla ? 'keypress' : 'keydown', this._keydown);
            }
        ...
        _addImage: function(index, src) {
            ...
            if (jq.browser.msie || this.retry.count) {
            ...
            .attr('src', src)
            .load(function() {
                self._imageLoaded(index)
            }).error(function() {
                self.retry.list.push(index)
            });
            this.imageList[index] = $newImg;
        ...
        _retryLoad: function() {
            ...
            this.retry.count++;
            jq.each(this.retry.list, function(ii, index) {
                self._addImage(index, images[index])
            });
            this.retry.list.length = 0;
            return this;
        },
        ...
        _mouseDrag: function(event) {
            ...
            if (($("#springer").css("display") == "block" || $(".zoom-box").css("display") == "block")) {
                return false;
            } //Ist der Springer sichtbar, dann funtion abschalten
